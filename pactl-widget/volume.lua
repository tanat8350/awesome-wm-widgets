...
local function build_rows(devices, on_checkbox_click, device_type)
	...
	for _, device in pairs(devices) do
		local checkbox = wibox.widget({
			...
			forced_width = 20,
			forced_height = 20,
			check_color = colors.one.lightgreen,
...
local function worker(user_args)
	...
	local function update_graphic(widget)
		local vol = string.format("%d", pactl.get_volume(device))
		if vol ~= nil then
	...
	local function getCurrentVol()
		return tonumber(string.format("%d", pactl.get_volume(device)))
	end

	local function volNoti_delete()
		if not (volNoti == nil) then
			naughty.destroy(volNoti)
		end
	end

	local function volNoti_notifyCurrentVol()
		currentVol = getCurrentVol()
		volNoti_delete()
		volNoti = naughty.notify({
			text = "Vol " .. currentVol,
			position = "top_middle",
		})
	end
	...
	function volume:inc(s)
		local currentVol = getCurrentVol()
		if currentVol >= maxVol then
			volNoti_delete()
			volNoti = naughty.notify({
				text = "Volume Max (" .. currentVol .. ")",
				position = "top_middle",
				border_color = colors.one.red,
			})
			return
		end
		pactl.volume_increase(device, s or step)
		update_graphic(volume.widget)
		volNoti_notifyCurrentVol()
	end
